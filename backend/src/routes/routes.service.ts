...
export class RoutesService {
  ...
  async listRoutes(type?: string) {
    ...
    const items = routes.map((r: any) => {
      const stops = r.routeStops?.map((rs: any) => rs.stop) ?? [];
      ...
      const activeBuses = r.buses?.filter((b: any) => b.status !== 'OUT_OF_SERVICE') ?? [];
  ...
  async getRouteEta(id: string) {
    ...
    const buses = (route.buses ?? []).map((b: any) => {
    ...
    const activeBuses = buses.filter((b: any) => b.status !== 'OUT_OF_SERVICE');
    const leadBus = activeBuses.length > 0
      ? activeBuses.reduce((a: any, b: any) => (a.currentStopIndex > b.currentStopIndex ? a : b))
      : null;

    const stops = (route.routeStops ?? []).map((rs: any) => {
